...
export default function App() {
  ...
  const addLink = () => {
    // 🛡️ LIMIT ACTIVATION: Cap at 3 for Free users
  ...
  const handleLaunch = async () => {
    ...
    try {
      ...
      setMessage("Server is currently offline. ❌");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-[#fafafa] flex flex-col items-center justify-center p-6 antialiased font-sans">
      <motion.button onClick={() => navigate('/login')} className="mb-8 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:text-black transition-all group">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Sources ({form.links.length}/3)</label>
              <button onClick={addLink} className="text-[10px] font-black text-black border-b-2 border-black">+ ADD SOURCE</button>
            </div>
            <div className="max-h-80 overflow-y-auto space-y-3 scrollbar-hide">
              <AnimatePresence mode="popLayout">
                {form.links.map((link, index) => (
                  <motion.div layout key={index} className="p-5 bg-gray-50 rounded-3xl space-y-4 border border-gray-100 group relative">
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <input placeholder="Label" value={link.platformName} className="bg-transparent outline-none text-sm font-bold" onChange={(e) => updateLink(index, 'platformName', e.target.value)} />
          ...
          <motion.button onClick={handleLaunch} disabled={loading} className="w-full py-5 bg-black text-white rounded-full font-black uppercase tracking-widest text-[11px] shadow-lg disabled:bg-gray-200">
